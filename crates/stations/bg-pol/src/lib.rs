//! BG-POL — 88.1 MHz
//!
//! Bulgarian politics and current affairs.
//! Sources: Mediapool, Capital, Dnevnik, Sega.
...
impl Guest for BgPolStation {
    fn describe() -> Metadata {
        Metadata {
            callsign:        "BG-POL".to_string(),
            name:            "BG Politics".to_string(),
            description:     "Bulgarian current affairs — Mediapool, Capital, Dnevnik, Sega.".to_string(),
            frequency:       "88.1".to_string(),
            operator:        "Pete".to_string(),
            cadence_seconds: 1800,
            declared_hosts:  vec![
                "www.mediapool.bg".to_string(),
                "www.capital.bg".to_string(),
                "www.dnevnik.bg".to_string(),
                "www.segabg.com".to_string(),
            ],
        }
    }
...
fn fetch_all() -> Result<Vec<RawItem>> {
    ...
    for (url, _label) in &[
        ("https://www.mediapool.bg/rss-news.xml",    "Mediapool"),
        ("https://www.capital.bg/rss/",              "Capital"),
        ("https://www.dnevnik.bg/politika/rss/",     "Dnevnik"),
        ("https://www.segabg.com/rss/category/1008", "Sega"),
    ] {
        match http_get(url).and_then(|b| parse_rss(&b)) {
            Ok(feed) => items.extend(feed),
            Err(_) => {} // partial failure is fine
        }
    }
    if items.is_empty() {
...
bindings::export!(BgPolStation with_types_in bindings);
